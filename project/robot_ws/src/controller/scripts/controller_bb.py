...
class Controller:
    ...
    def __init__(self, state, platoons, robot):
        ...
        self.laneSwitch_pub = rospy.Publisher(robot+'/arduino/laneswitch', Int16, queue_size=-1)
        self.left_ir = 0.0
        ...
        for i in range(len(states)):
            ...
            if i in self.platoons[self.state['platoon']]['robots']:
                ...
                if (self.state['mode'] == SIDE_FORMATION_MODE):
                    ...
                    if self.state['platoon_pos'] > 1:
                        if abs(polar_r - self.state['polar_r']) < 35 and abs(polar_angle - self.state['polar_angle']) < pi/6:
                            self.break_applied = abs(polar_angle - self.state['polar_angle'])/(pi/6)
    ...
    def action_cb(self,data):
        ...
        if(action_id == 'a'):
            ...
            if self.busy is True:
                ...
                self.feedback_pub.publish("1,NOTOK")
            elif sender == 0 and self.busy is False and self.state['platoon_pos'] == 1:
                self.busy = True
                commandMsg = "" + str(rospy.get_time()) + ",f,0,0,1,1"
                self.action_pub.publish(commandMsg)
            elif self.state['platoon_pos'] != 1 and sender == 1:
                self.linefollow_pub.publish(-1)
                self.feedback_pub.publish("1,OK")
                if self.state['mode'] == LINE_FOLLOWING_MODE:
                    self.state['mode'] = LISTENING_MODE
                    if(self.state['lane'] == 0):
                        ...
                        self.laneSwitch_pub.publish(0)
                        rospy.sleep(3)
                        self.state['mode'] = LINE_FOLLOWING_MODE
                elif self.state['mode'] == SIDE_FORMATION_MODE:
                    #self.busy = True
                    self.linefollow_pub.publish(50)
                    rospy.sleep(5)
                    self.state['speed'] = (50,50)
                ...
                self.feedback_pub.publish("1,DONE")
                self.busy = False
            else:
                print("not leader! cannot chnge pos!")
                self.feedback_pub.publish("1,NOTOK")
        elif(action_id == 'g'):
            if not self.busy:
            ...
            if self.busy:
                ...
                if newMode == LINE_FOLLOWING_MODE or newMode == SIDE_FORMATION_MODE:
                    self.linefollow_pub.publish(35)
                    self.state['speed'] = (35,35)
    ...
    def feedback_cb(self,data):
        ...
        if int(params[0]) == self.state['ID']:
            if params[1] == "NOTOK":
                print("could not change other robot! leader remains")
                self.busy = False
            elif params[1] == "OK" and (self.state['mode'] == LINE_FOLLOWING_MODE or self.state['mode'] == LINE_FOLLOWING_MODE):
                print("here")
                self.linefollow_pub.publish(-1)
            elif params[1] == "DONE":
...
if __name__ == "__main__":
    rospy.init_node('bb_controller', anonymous=True)
    robot = 'bigboy'
    tinyboy_state = {
        'ID':1, 
        'platoon':0, 
